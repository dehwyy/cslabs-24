...
const str::String CIPHER_FILE = str::FromStaticCStr("cipher");
...
bool IsFilledStruct(ParsedFilenames f) {
    return str::Len(f.input_file) && str::Len(f.cipher_file) && str::Len(f.encoded_output_file) && str::Len(f.decoded_output_file);
}
...
ParsedFilenames parse(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (str::Equals(key, CIPHER_FILE)) {
            args.cipher_file = value;
